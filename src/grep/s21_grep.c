...
int main(int argc, char *argv[]) {
  ...
  int num_files = 10;  // Например, количество строк в массиве
  char **files = malloc(num_files * sizeof(char *));
  if (files == NULL) {
    printf("memory for the array cannot be allocated");
  }
  Flags flags = GrepPars(argc, argv, patterns, files);
...
Flags GrepPars(int argc, char *argv[], char *patterns, char **files) {
  ...
  ProcArgs(argc, argv, &flags, patterns, files);
  return flags;
}
